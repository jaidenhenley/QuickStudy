...
struct QuizView: View {
    ...
    private var startScreen: some View {
        Form {
            ...
            Section {
                ...
                .disabled(viewModel.quizQuestions.isEmpty || viewModel.isGeneratingQuiz)
            } footer: {
                if viewModel.quizQuestions.isEmpty {
                    Text("Approve at least one card in this set to start.")
                } else {
                    Text("Quiz answers are generated by Apple Intelligence.")
                }
            }
        }
    ...
    private func prepareQuestions() async {
        // Try AI-generated distractors first, fall back to pool-based method
        await viewModel.generateAIQuizQuestions()

        var questions = viewModel.aiQuizQuestions ?? viewModel.quizQuestions
        if shuffleQuestions {
            questions.shuffle()
        }
        switch questionCount {
